...
public class DemoSynchronizeMode {
  ...
  public DemoSynchronizeMode() throws EclipseException, IOException {
    // <ECLiPSe 7.1 dir>
    File eclipseHome = new File("C:\\Program Files\\ECLiPSe 7.1");
  ...
  public static void main(String[] args) {
    Path eclpath = Paths.get(System.getProperty("user.dir"), "test", "ccu", "pllab", "tcgen3",
        "core", "testcasegen", "solver", "test", "example.ecl");
    File eclfile = eclpath.toFile();

    Path eclpath2 = Paths.get(System.getProperty("user.dir"), "test", "ccu", "pllab", "tcgen3",
        "core", "testcasegen", "solver", "test", "example2.ecl");
   ...
   * 
   * EclipseEngine eng = EmbeddedEclipse.getInstance( new EclipseEngineOptions(new
   * File("C:\\ECLiPSe"))); try { eng.rpc("writeln('before-crash'), flush(output)."); // 這行會讓 *同一個
   * JVM 進程* 直接執行 exit(99) → eng.rpc("exit(99)."); // ← 99 只是示意 } catch (Exception t) { // 連線被關會拋
   * Throw System.err.println("子行程已結束，JVM 繼續跑！"); t.printStackTrace(); } EclipseEngine eng2 =
   * EmbeddedEclipse.getInstance( new EclipseEngineOptions(new File("C:\\ECLiPSe")));
   * eng2.rpc("lib(ic).");
   * System.out.println(eng2.rpc("writeln('New process OK'), flush(output)."));
   * 
   * // **以下程式永遠不會被執行** System.out.println("你看不到我"); }
   */
}
