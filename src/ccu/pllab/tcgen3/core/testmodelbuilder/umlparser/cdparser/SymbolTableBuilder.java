...
public class SymbolTableBuilder {
	...
	private void processAttributes(Element classElement, ClassSymbol classSymbol, String classname) {
	    ...
	    for (int j = 0; j < ownedAttributes.getLength(); j++) {
	        ...
	        if (attrNode.getNodeType() == Node.ELEMENT_NODE &&
	             ...
	             * {lowerValue, upperValue, id}, if upperValue is '*' then id change*/
	 ...
	 * <p>
	 * • If the string is not properly wrapped, an IllegalArgumentException is thrown.  
	 * • If the braces contain no numbers, the method returns 1.  
	 *
	...
	private int multiplyIfBraced(String text) {
	    ...
	    return Arrays.stream(content.split(","))
	                 .map(String::trim)          // Remove optional whitespace around numbers
	...
	private String extractDefaultValue(Element attrElement) {
        ...
        if (defaults.getLength() > 0) {
            ...
            if ("uml:LiteralInteger".equals(dvType) ||
                "uml:LiteralString".equals(dvType)  ||
