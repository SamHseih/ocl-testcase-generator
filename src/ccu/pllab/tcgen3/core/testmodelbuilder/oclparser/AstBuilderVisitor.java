 ...
 * performing *basic* semantic checks (duplicate declarations, unknown references, simple type
 * lookup).<br>
 * <br>
 * <b>Important design decisions</b>
 * <ul>
 * <li>We <em>never</em> mutate the <code>Scope</code> instance passed from the UML-side directly.
 * Instead, each OCL context that introduces a block (package, context, iterate, let, etc.) creates
 * its own {@link BaseScope} whose <code>enclosingScope</code> is whatever is on the top of
 * {@link #currentScope}. This keeps look-ups correct while ensuring we do not pollute the global
 * table.</li>
 * <li>All {@code visitXXX} methods return an {@link ASTree}. The concrete subclasses live under
 * <code>ccu…ast.impl</code>. If a particular grammar rule does not contribute to the final AST we
 * simply delegate to {@link #visitChildren} and return the first child.</li>
 * <li>Minimal, practical error-reporting: discovered problems are collected in
 * {@link #semanticErrors}. Parsing continues so that we can show <em>all</em> errors to the user in
 * one pass.</li>
 * </ul>
 */
public class AstBuilderVisitor extends OclBaseVisitor<ASTree> {
  ...
  // private boolean insidePre = false; //原本是只要在 precondition 都掛上pre 但OCL side-effect-free 其實不需要!
  // 待刪除

  /*
   * 1. duplicate declaration 2. reference to undeclared variable. Note that semantic errors are
  ...
  private void recordError(String msg, Token t) {
    semanticErrors
        .add(String.format("%s (%d:%d)", msg, t.getLine(), t.getCharPositionInLine() + 1));
  }

  /*
   * ===================================================== Package Syntax.. packageDeclarationCShg :
   * 'package' pathNameCS ownedMemberDeclaration* 'endpackage' ;
   * =====================================================
   */
  @Override
   ...
   * classifierContextDecl context* | operationContextDecl context* ;
   * =====================================================
   */
  @Override
  public ASTree visitContextDecl(ContextDeclContext ctx) {
    ASTree declcontext = (ctx.classifierContextDecl() != null ? visit(ctx.classifierContextDecl())
        : visit(ctx.operationContextDecl()));
   ...
   * pathNameCS '::' ID '(' contextOpParameters? ')' # NoRturnTypeAlt | 'context' pathNameCS '::' ID
   * '(' contextOpParameters? ')' ':' type # HasRturnTypeAlt ;
   * =====================================================
   */
  @Override
  ...
  public ASTree visitHasRturnTypeAlt(HasRturnTypeAltContext ctx) {
    ...
    if (ret.contains("[]")) {
      if (!(returntype instanceof ArrayTypeClassSymbol)) {
        recordError("OperationContestDecl Error: UML or Ocl defined different return type " + ret,
            rtk);
      }
    }
  ...
  /*
   * ===================================================== context : 'inv' ID? ':' oclExpressionCS |
   * 'pre' ID? ':' oclExpressionCS | 'post' ID? ':' oclExpressionCS ;
   * =====================================================
   */
  @Override
  ...
  public ASTree visitVariableDeclAlt(VariableDeclAltContext ctx) {
    ...
    if (currentScope() instanceof LocalScope) {
      if (currentScope().resolve(varName) != null) {
        recordError("VariableDecl Error: Duplicate declaration of variable '" + varName + "'",
            idToken);
      }
  ...
  public ASTree visitCollectionDeclAlt(CollectionDeclAltContext ctx) {
    ...
    if (currentScope() instanceof LocalScope) {
      if (currentScope().resolve(collectionName) != null) {
        recordError(
            "VariableDecl Error: Duplicate declaration of variable '" + collectionName + "'",
            idToken);
      }
      if (typeSym == null || !(typeSym instanceof Type)) {
        recordError("VariableDecl Error: Unknown type '" + typeName + "'", idToken);
      ...
      recordError(
          "CollectionDecl Error: CollectionDecl only for LocalScope'" + collectionName + "'",
          idToken);
      return null;
    }
   ...
   *------------------------------------------------------------------*/
  @Override
  public ASTree visitNonCallExp(NonCallExpContext ctx) {// primaryExpression ifExpCs let...
    if (ctx.primaryExpression().expression() != null) {
      return visit(ctx.primaryExpression().expression());
    } else
  ...
  public ASTree visitVarFeatureCallAlt(VarFeatureCallAltContext ctx) {
    ...
    if (isPre && !insidePost) {
      recordError("@pre can only be used inside postconditions",
          ctx.variableExpCS().isMarkedPreCS().stop);
    }
    Symbol sym = currentScope().resolve(varname);
    if (sym == null) {
      recordError("Variable FeatureError: UnDefined variable '" + varname + "'",
          ctx.variableExpCS().getStart());
      return new InvalidAST();
    }
  ...
  public ASTree visitIterateCallAlt(IterateCallAltContext ctx) {
    ...
    if (current == null) {
      recordError("CollectionLiteralIterateCall Error: UnDefined CollectionLiteral source ",
          ctx.getStart());
    }
  ...
  public ASTree visitImpliciteOpCallAlt(ImpliciteOpCallAltContext ctx) {
    ...
    if (existing == null) {
      recordError("ImpliciteOpCall Error: 'Implicite self' used outside classifier context",
          ctx.getStart());
    }
  ...
  public ASTree visitClassFeatureCallAlt(ClassFeatureCallAltContext ctx) {
    ...
    if (existing == null) {
      recordError("Self FeatureCall Error: 'self' used outside classifier context",
          ctx.SELF().getSymbol());
    }
  ...
  public ASTree visitResultFeatureCallAlt(ResultFeatureCallAltContext ctx) {
    ...
    if (!(s instanceof MethodSymbol method)) {
      recordError("Result FeatureCall Error : 'result' used outside any operation context",
          ctx.getStart());
      return new InvalidAST();
    }
    ...
    if (resultSym == null) {
      recordError("Result FeatureCall Error : 'result' used outside classifier context",
          ctx.getStart());
    }
    /* 4. 基底節點 */
  ...
  public ASTree visitVarArrayAccessAlt(VarArrayAccessAltContext ctx) {
    ...
    Symbol sym = currentScope().resolve(varname);
    if (sym == null)
      recordError("Variable Array Access Error : UnDefined array " + varname,
          ctx.variableExpCS().start);
    Type t;
    if (sym instanceof ArrayTypeClassSymbol a) {
      ...
      if (b.getType() instanceof ArrayTypeClassSymbol a) {
        ...
        recordError(
            "Variable Array Access Error : Variable " + varname + " isn't ArrayTypeClassSymbol ",
            ctx.variableExpCS().start);
      }
    }
    ...
    for (int i = 0; i < ctx.indexExpCS().size(); i++) {
      ...
      if (index == null) {
        recordError("Variable Array Access Error : UnDefined array index ",
            ctx.variableExpCS().start);
        break;
      }
      indices.add(index);
  ...
  public ASTree visitClassArrayAccessAlt(ClassArrayAccessAltContext ctx) {
    ...
    Symbol existing = s.resolve("self");
    if (existing == null)
      recordError("Self Array Access Error : 'self' used outside classifier context",
          ctx.SELF().getSymbol());
    if (!(existing instanceof ArrayTypeClassSymbol))
      recordError("Self Array Access Error : 'self' isn't ArrayTypeClassSymbol",
          ctx.SELF().getSymbol());
  ...
  public ASTree visitResultArrayAccessAlt(ResultArrayAccessAltContext ctx) {
    ...
    if (!(s instanceof MethodSymbol method)) {
      recordError("Result Array Access Error : 'result' used outside any operation context",
          ctx.getStart());
      return new InvalidAST();
    }
    Symbol resultSym = method.resolve("result");
    if (resultSym == null)
      recordError("Result Array Access Error : 'result' used outside classifier context",
          ctx.getStart());
    if (!(resultSym instanceof ArrayTypeClassSymbol))
      recordError("Self Array Access Error : 'self' isn't ArrayTypeClassSymbol",
          ctx.RESULT().getSymbol());
  ...
  private ASTree buildCallExp(CallExpCSContext ctx, ASTree source) {
    if (ctx.featureCallExpCS() != null) {
      ...
      return buildLoopCall(ctx.loopExpCS(), source);
  ...
  private ASTree buildOperationCall(OperationCallExpCSContext ctx, ASTree source) {
    ...
    if (source instanceof VariableExp) {
      ...
      if (sym instanceof ClassSymbol) {
        ...
        if (methodSym == null) {
          recordError("Build OperationCall Error: UnDefined MethodSymbol " + opname + "in"
              + classSym.getName(), ctx.getStart());
          returnType = new InvalidType();
        }
        if (methodSym.getType() != null) {
          if (methodSym.resolveType(methodSym.getType().getTypeName()) == null) {
            recordError("Build OperationCall Error: UnDefined returnType "
                + methodSym.getType().getTypeName() + "in" + opname, ctx.getStart());
            returnType = new InvalidType();
          }
          sym = methodSym;
        ...
        recordError("Build OperationCall Error: Source must be Class " + opname + " in " + source,
            ctx.getStart());
        returnType = new InvalidType();
      }
      ...
      if (sym instanceof ClassSymbol classSym) {
        MethodSymbol methodSym = classSym.resolveMethod(opname);
        if (methodSym == null) {
          recordError("Build OperationCallExp Error: UnDefined Method " + opname + " in "
              + classSym.getName(), ctx.getStart());
          returnType = new InvalidType();
        }
        if (methodSym.getType() != null) {
          if (methodSym.resolveType(methodSym.getType().getTypeName()) == null) {
            recordError("Build OperationCallExp Error: UnDefined returnType "
                + methodSym.getType().getTypeName() + " in " + opname, ctx.getStart());
            returnType = new InvalidType();
          }
          returnType = methodSym.getType();
        ...
        if (type instanceof ClassSymbol) {
          ...
          if (methodSym == null) {
            recordError("Build OperationCallExp Error: UnDefined Method " + opname + " in "
                + ((ClassSymbol) type).getName(), ctx.getStart());
            returnType = new InvalidType();
            sym = methodSym;
          }
          if (methodSym.getType() != null) {
        ...
        recordError(
            "Build OperationCall Error: UnDefined source operationCall " + opname + "in" + source,
            ctx.getStart());
        returnType = new InvalidType();
      }
    ...
    if (sym instanceof MethodSymbol m) {
      if (args.size() != m.getNumberOfSymbols()) {
        recordError(
            "Build OperationCall Error: Parameter Error " + " in " + source + " call " + opname,
            ctx.getStart());
      }
    }
            ...
            if (indexValue.getValue() > DefinedDimension || indexValue.getValue() < 0) {
              recordError(
                  "Dimension is " + DefinedDimension
                      + "\nBuild OperationCall Error:Call Array dimensionSizes out of range "
                      + indexValue.getValue() + " in " + source + " call " + opname,
                  ctx.getStart());
            } else if (DefinedDimension <= 0) {
              recordError(
                  "Array Error: Array dimensionSizes out of range, and dimension need to set >=1 ",
                  ctx.getStart());
            }
          }
        }
    ...
    return new OperationCallExp(source, args, returnType, isPre, opname,
        ctx.ARROW() != null ? "->" : ".", sym);
  }
  ...
  private ASTree buildPropertyCall(PropertyCallExpCSContext ctx, ASTree source) {
    ...
    if (!(sourcetype instanceof ClassSymbol)) {
      recordError("Build PropertyCall Error: Source Type " + source + " isn't Class Type",
          ctx.variableExpCS().isMarkedPreCS().stop);
    } else if (sourcetype instanceof ClassSymbol c) {
      propsym = c.resolve(propName);
      if (propsym == null) {
        recordError("Build PropertyCall Error: UnDefined property " + propName + " in " + c,
            ctx.getStart());
      }
      ...
      if (propsym instanceof BaseSymbol) {
        ...
        if (proptype == null) {
          recordError(
              "Build PropertyCall Error: UnDefined type " + propName + " in " + currentScope(),
              ctx.getStart());
        }
  ...
  private ASTree buildArrayAccessCall(ArrayAccessCSContext ctx, ASTree source) {
    ...
    if (isPre && !insidePost) {
      recordError("@pre can only be used inside postconditions",
          ctx.variableExpCS().isMarkedPreCS().getStart());
    }
    Symbol sym = null;
    ...
    if (source instanceof VariableExp) {
      ...
      if (sourcesym instanceof ClassSymbol) {
        ...
        if (sym instanceof ClassSymbol)
          if (!(sym instanceof ArrayTypeClassSymbol)) {
            recordError(
                "Build ArrayFeatureCall Error: " + arrayName + " must be ArrayTypeClassSymbol",
                ctx.getStart());
          }
        ...
        recordError("Build ArrayFeatureCall Error: Source must be ClassSymbol "
            + ((VariableExp) source).getName(), ctx.getStart());
      }
      ...
      if (type instanceof ArrayTypeClassSymbol) {
        ...
        if (sym == null) {
          recordError("Build ArrayFeatureCall Error: ArrayTypeClassSymbol "
              + ((FeatureCallExp) source).getFeatureName() + " does't have MultiplicityListType: "
              + arrayName, ctx.getStart());
        }
        if (!(sym instanceof ArrayTypeClassSymbol)) {
          recordError("Build ArrayFeatureCall Error: " + arrayName + " isn't ArrayTypeClassSymbol",
              ctx.getStart());
        }
      } else {
        recordError("Build ArrayFeatureCall Error: ArraySource must be ArrayTypeClassSymbol "
            + ((FeatureCallExp) source).getFeatureName(), ctx.getStart());
      }
    }
  ...
  private ASTree buildLoopCall(LoopExpCSContext ctx, ASTree source) {
    ...
    boolean isPre = false;
    Symbol sym = null;
    if (source instanceof VariableExp v) {
      isPre = v.isMarkedPre();
      sym = v.getSymbol();
    }
    if (source instanceof FeatureCallExp f) {
      isPre = f.isMarkedPre();
      sym = f.getSymbol();
    }
    ...
    return new IterateExp(List.of(source, iteratorDecl, resultDecl, body), isPre, sym);
  }
  ...
  public ASTree visitPrimitiveLiteralExpCS(PrimitiveLiteralExpCSContext ctx) {
    /* 找最近的 global scope */
    Scope s = currentScope();
    while (s != null && !(s instanceof GlobalScope))
      s = s.getEnclosingScope();
  ...
  /*
   * ===================================================== Default/fallback – delegate to children
   * so we do not need to implement every rule.
   * =====================================================
   */
