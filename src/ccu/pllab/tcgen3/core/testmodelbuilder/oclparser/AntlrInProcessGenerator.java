...
/**
 * Utility driver that invokes the ANTLR&nbsp;v4 code generator <em>inside the
 * current JVM</em> instead of spawning an external process.
 * <p>
 * The class resolves both the grammar file (<code>Ocl.g4</code>) and the
 * destination directory for generated sources relative to the project root
 * (obtained from the <code>user.dir</code> system property). It then calls
 * {@link org.antlr.v4.Tool} programmatically with arguments equivalent to the
 ...
 * </pre>
 *
 * After successful generation a single confirmation line is printed to standard
 * output.  Any runtime errors thrown by ANTLR will propagate to the console,
 * facilitating debugging inside the IDE.
 * 
 * consider {@link AntlrCliGenerator} for a CLI-based approach.
 */
public class AntlrInProcessGenerator {
     ...
     * <p>
     * The method constructs the argument array required by
     * {@link org.antlr.v4.Tool} to
     * <ul>
     ...
     * </ul>
     * Finally, it calls
     * {@link org.antlr.v4.Tool#processGrammarsOnCommandLine()} which performs
     * the actual generation. The <code>args</code> parameter is currently
     * ignored but kept for future extension.
     *
