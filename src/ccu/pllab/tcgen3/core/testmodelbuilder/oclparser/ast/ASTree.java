...
public abstract class ASTree implements Iterable<ASTree>, Expression {
  ...
  /**
   * @return returns a string that represents the position of an abstract syntax tree (AST) node
   *         within the program.
   */
  ...
  /**
   * 走訪 OCL 抽象語法樹（AST）並逐步建構 {@link ccu.pllab.tcgen3.core.testmodelbuilder.clg.CLGGraph CLG}
   * 的訪客介面。<br>
   * <p>
   * 型別參數 {@code <R>} 為各 {@code visit*Context} 方法的回傳型別。<br>
   * 常見做法是回傳整張 {@code CLGGraph}，亦可依需求改成 {@code Boolean}、{@code Optional<CLGGraph>}、{@code Void} … 等。
   * </p>
   *
   * <p>
   * <strong>實作要點：</strong><br>
   * - 為「需要特別處理」的每一種 AST 節點撰寫對應的 {@code visitXxxContext} 方法。<br>
   * - 不需處理的型別可落回 {@link #visitDefault(ASTree)}。<br>
   * - 若在走訪途中偵測到語意錯誤，請透過 {@link #getErrorMesg()} 收集， 方便呼叫端一次檢視所有問題。
   * </p>
   *
   * <h2>使用範例</h2>
   * 
