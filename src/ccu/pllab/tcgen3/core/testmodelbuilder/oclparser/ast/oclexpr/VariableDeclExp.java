...
public class VariableDeclExp extends ASTLeaf implements Expression {
  ...
  /**
   * VariableDeclExp ├── child(0) : expressionExp (for var init)
   */
  ...
  public ASTree clone() {
    /*
     * // 1.深層複製初始化運算式 (varinit) java.util.List<ASTree> clonedInit = new ArrayList<>(); for (ASTree
     * child : this.children) { clonedInit.add(child.clone()); }
     * 
     * // 2. 使用複製後的 child 清單，建立新 VariableDeclExp // - varname 為 String（immutable）→ 直接帶入 // - vartype
     * 為 Type（依先前約定不可變）→ 直接共用即可
     */
