...
/**
 * Invokes the ANTLR&nbsp;v4 code‑generator **via an external PowerShell process** so that
 * the grammar can be rebuilt with a single <em>Run</em> action inside Eclipse.
 * <p>
 * The class constructs and executes the following command (line breaks added
 * for readability):
 * </p>
 *
 * <pre>{@code
 ...
 * </pre>
 * <p>
 * The paths are resolved at runtime using the {@code user.dir} system property;
 * adjust {@link #ANTLR_JAR_WITH_MAIN} or the source / output directories if the
 * layout changes.
 * </p>
 *
 ...
 * </ul>
 * <strong>Limitations:</strong> the implementation is platform‑specific (Windows only)
 * and incurs the overhead of spawning a new JVM.  For cross platform builds
 * consider {@link AntlrInProcessGenerator} or a build-tool plugin instead.
 */
public class AntlrCliGenerator {
     ...
     * <p>
     * <strong>Note:</strong> keep the space between the JAR path and the main
     * class name; it is required by the PowerShell <code>java -cp</code>
     * invocation.
     * </p>
     */
    ...
    /**
     * Spawns the external process, streams its combined <em>stdout/stderr</em>
     * to the console, and prints the resulting exit code.
     *
     * @param args currently unused — reserved for future parameters such as
     *             alternative grammar paths or output directories
     */
