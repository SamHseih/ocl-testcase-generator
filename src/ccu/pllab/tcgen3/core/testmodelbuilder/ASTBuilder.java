...
public class ASTBuilder {
  ...
  public void genASTGraph() {
    ...
    for (int i = 0; i < p.numChildren(); i++) {
      ...
      Path dotpath = projectpath.resolve("output").resolve("AST").resolve(p.getClassName())
          .resolve(dotfilename);
      Path svgpath = projectpath.resolve("output").resolve("AST").resolve(p.getClassName())
          .resolve(svgfilename);
    ...
    Path dotpath =
        projectpath.resolve("output").resolve("AST").resolve(p.getClassName()).resolve(dotfilename);
    Path svgpath =
        projectpath.resolve("output").resolve("AST").resolve(p.getClassName()).resolve(svgfilename);
