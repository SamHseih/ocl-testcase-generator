 ...
 * </ul>
 * <p>
 * Thread‑safe for read‑heavy workloads: structural mutations guarded by "synchronized"; look‑ups
 * rely on concurrent collections.
 */
public final class CLGGraph {
  ...
  public synchronized void addEdge(CLGEdge e) {
    ...
    if (!nodes.containsKey(e.getFrom().id()) || !nodes.containsKey(e.getTo().id())) {
      throw new IllegalArgumentException(
          "Both endpoint nodes must be present in graph before adding edge");
    }
  ...
  // for clone
  public void setStart(CLGNode start) {
    if (start.getType() != CLGNodeType.START) {
      throw new IllegalArgumentException("Start node must be of type START");
    }
    this.start = start;
  }

  // for clone
  public void setEnd(CLGNode end) {
    if (end.getType() != CLGNodeType.END) {
      throw new IllegalArgumentException("End node must be of type END");
    }
    this.end = end;
  }
  ...
  // Deep clone of the graph, including nodes and edges.
  ...
  /**
   * Basic sanity checks: single start/end, connectivity from start to every node, no dangling
   * edges.
   */
  public void validate() {
    ...
    if (reachable.size() != nodes.size()) {
      throw new IllegalStateException("Graph contains unreachable nodes. Reachable="
          + reachable.size() + ", total=" + nodes.size());
    }
  }

  private void dfs(CLGNode n, Set<CLGNode> vis) {
    if (!vis.add(n))
      return;// Set not allow duplicate
  ...
  /*
   * ===================================================== G R A P H O P E R A T I O N S I M P L E M
   * E N T A T I O N =====================================================
   */
  public void graphAnd(CLGGraph rhs) {
    ...
    // CLGEdge graphand = thisConeected.connectTo(rhsConnected, CLGEdgeType.SEQUENTIAL, "graphAnd",
    // -1);
    this.addEdge(
        thisLastConstraint.connectTo(rhsfirstConstraint, CLGEdgeType.SEQUENTIAL, "graphAnd", -1));
  ...
  public void graphClosure() {
    ...
    CLGEdge ITERATION =
        branchnode.connectTo(thisFirstConstraint, CLGEdgeType.ITERATION, "iterate", -1);
    CLGEdge ITERATION_MERGE =
        thisLastConstraint.connectTo(branchnode, CLGEdgeType.ITERATION_MERGE, "iterate", -1);
