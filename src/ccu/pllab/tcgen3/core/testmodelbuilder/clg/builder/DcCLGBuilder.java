...
public class DcCLGBuilder<T> implements AstVisitor<CLGGraph> {
  ...
  public CLGGraph visitContextDeclAST(ContextDeclAST node) {
    ...
    CLGGraph excepclg = null;// for false pre condition CLG used
    ...
    if (node.getContextDecl() instanceof OperationContextDeclAST op) {
      ...
      for (int i = 1; i < node.numChildren(); i++) {
        if (node.child(i) instanceof ContextExpAST ctx) {
          if (ctx.getKeyWord().equals("pre")) {
            ...
            String excepName = ctx.getExceptionlabel();
            if (!excepName.isEmpty())
              excepclg = new CLGGraph(new ExceptionAST(excepName));
            else
              excepclg = new CLGGraph(new ExceptionAST());
            // true pre condition
      ...
      for (int i = 1; i < node.numChildren(); i++) {
        if (node.child(i) instanceof ContextExpAST ctx) {
          ...
          CLGtransferMessege
              .add("ClassifierContextDecl Error! There isn't appropriate AST to CLG.");
        }
      }
    ...
    if (!notpreclgs.isEmpty()) {
      ...
      if (excepclg != null)
        ...
        CLGtransferMessege.add("There is not pre-condition CLG, but no exception CLG.");
      }
    }
  ...
  public CLGGraph visitBinaryExpContext(BinaryExp node) {
    ...
    if (node.left() instanceof IfExp || node.left() instanceof IterateExp
        || node.right() instanceof LetExp || node.left() instanceof LetExp) {
      leftclg = node.left().accept(this);
    }
    if (node.right() instanceof IfExp || node.right() instanceof IterateExp
        || node.right() instanceof LetExp || node.left() instanceof AccumulatorExp// Special Case :
                                                                                  // Ex.acc =
                                                                                  // <OclExpr>
    ) {
  ...
  public CLGGraph visitIterateExpContext(IterateExp node) {
    ...
    IndexVariableExp Index =
        new IndexVariableExp("idx" + nth_ITERATE++, (Symbol) SymbolTableBuilder.IntType);
    // OCL && ECLiPSe index init from 1
    BinaryExp initIndex =
        new BinaryExp(List.of(Index, new IntegerLiteralExp(1, SymbolTableBuilder.IntType)), "=");
    ...
    if (node.getSource() instanceof CollectionPart c) {
      // collection size n from CollectionPart Method size() , CollectionRange/CollectionItem
      // extends CollectionPart
  ...
  public CLGGraph visitLetExpContext(LetExp node) {
    ...
    if (!declclgs.isEmpty()) {
      ...
      CLGtransferMessege.add("Inv Context Err.");
    }
  ...
  public CLGGraph visitVariableDeclExpContext(VariableDeclExp node) {
    if (node.numChildren() == 0) {
      ...
      if (node.child(0) instanceof ASTLeaf || node.child(0) instanceof OperationCallExp
          || node.child(0) instanceof ArrayRefExp || node.child(0) instanceof PropertyCallExp) {
        return new CLGGraph(node);
      } else {
