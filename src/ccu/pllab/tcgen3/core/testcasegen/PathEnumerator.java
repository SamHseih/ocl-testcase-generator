...
/**
 * <strong>PathEnumerator</strong> — an <em>interactive</em> enumerator that generates complete
 * paths from {@code START → … → END} within a {@link CLGGraph}.
 * <p>
 * Key features:
 * <ul>
 * <li>Implements {@link Iterable}{@code <List<CLGEdge>>}, allowing you to obtain one path at a time
 * via {@code for-each} or an explicit {@code Iterator}.</li>
 * <li>Uses <em>Breadth-First Search</em> (BFS) so that shorter paths are produced first.</li>
 * <li>Supports loop edges ({@link CLGEdgeType#ITERATION}); the number of times an iteration edge
 * can be unrolled in a single path is bounded by {@code maxUnroll}.</li>
 * <li>Still provides <code>enumerate()</code> and <code>stream()</code> batch / lazy APIs; both
 * delegate to the underlying iterator.</li>
 * </ul>
 * <h2>Basic usage</h2>
 * 
 * <pre>{@code
 * PathEnumerator pe = new PathEnumerator(clg, 1000, 3); // up to 1,000 paths, unroll loops at most
 *                                                       // 3 times
...
public final class PathEnumerator implements Iterable<List<CLGEdge>> {
   ...
   * @param maxPaths the maximum number of paths to enumerate (use {@link Integer#MAX_VALUE} for no
   *        limit)
   * @param maxUnroll the maximum number of times an {@code ITERATION} edge may be unrolled in a
   *        single path (must be ≥ 0)
   */
  public PathEnumerator(CLGGraph graph, int maxPaths, int maxUnroll) {
    ...
    if (maxUnroll < 0)
      throw new IllegalArgumentException("maxUnroll must be ≥ 0");
