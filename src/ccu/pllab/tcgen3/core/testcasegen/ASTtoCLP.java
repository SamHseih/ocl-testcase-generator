...
public class ASTtoCLP<T> implements AstVisitor<ClpCode> {
  ...
  public ClpCode visitOperationCallExpContext(OperationCallExp node) {
    ...
    while (current != null) {
      if (current instanceof FeatureCallExp f) {
        sourcenames.push(f.getFeatureName());
        current = f.getSource();
        // SelfExp、ResultExp 、VariableExp Case
      } else if (current instanceof SelfExp v) {// source root case 1
        sourcenames.push(v.getName());
        break;
      } else if (current instanceof ResultExp v) {// source root case 2
        sourcenames.push(v.getName());
        break;
        ...
        if (this.currentScope.resolve(v.getName()) instanceof ParameterSymbol p) {
          ...
          sourcenames.push(v.getName());
          break;
        } else if (this.currentScope.resolve(v.getName()) instanceof FieldSymbol) {
          // FieldExp Case
          sourcenames.push("Self_" + v.getName());
          break;
        } else {
          // VariableExp Case
          sourcenames.push(v.getName());
    ...
    ClpCode predifinedOp = switch (node.getFeatureName()) {
      case "getDimension", "getDimensionSizes", "getElements", "Size", "mod" -> predefinedfunc(node,
          sourcename);
      default -> null;
    };
  ...
  private ClpCode predefinedfunc(OperationCallExp node, String source) {
    ...
    String sourceStr =
        readVar(StringTool.tocapitalizeFirstLetter(source), node.getSource().getType());
    ...
    if (getVariableSource.getSource() instanceof VariableExp v)
      rootSourceStr = v.getName();
    ...
    String method = switch (funcName) {
      ...
      case "getDimension" -> "length(" + StringTool.tocapitalizeFirstLetter(rootSourceStr)
          + "_dimensionSizes" + "," + ssaVar + "),\n";
      case "getDimensionSizes" -> {
        if (args.isEmpty()) {
          // yield "getDimensionSizes(" + sourceStr + ", " + ssaVar + "),\n";
          yield StringTool.tocapitalizeFirstLetter(rootSourceStr) + "_dimensionSizes" + " = "
              + ssaVar + "\n";
        } else if (args.size() == 1) {
          if (args.get(0) instanceof IntegerLiteralExp i) {
            ...
            yield "nth1(" + index + "," + StringTool.tocapitalizeFirstLetter(rootSourceStr)
                + "_dimensionSizes" + "," + ssaVar + "),\n";
          } else {
            ...
            yield "nth1(" + idxExpr + "," + StringTool.tocapitalizeFirstLetter(rootSourceStr)
                + "_dimensionSizes" + "," + ssaVar + "),\n";
          }
        } else {
      ...
      case "getElements" -> "getElements(" + StringTool.tocapitalizeFirstLetter(rootSourceStr)
          + ", " + ssaVar + "),\n";
      // delay_mod(-Input,-Integer,+Output). Ex. Input = 100, Integer=6, Output=4
      case "mod" -> {
        int integer = 1;// default Input, Output = 0.
        if (node.child(1) instanceof IntegerLiteralExp i)
          integer = i.getValue();
        yield "delay_mod(" + StringTool.tocapitalizeFirstLetter(sourceStr) + "," + integer + ", "
            + ssaVar + "),\n";
      }
      case "Size" -> "Size(" + StringTool.tocapitalizeFirstLetter(sourceStr) + ", " + ssaVar
          + "),\n"; // 假設 Size
  ...
  public ClpCode visitArrayRefExpContext(ArrayRefExp node) {
    ...
    String sourcename =
        readVar(StringTool.tocapitalizeFirstLetter(sb.toString()), node.getSource().getType());
    ...
    for (int i = 1; i < node.numChildren(); i++) {
      ...
      func.append(readVar(StringTool.tocapitalizeFirstLetter(node.child(i).toString()),
          SymbolTableBuilder.IntType));
  ...
  public ClpCode visitCollectionRangeContext(CollectionRange node) {
    ...
    String Index = this.readVar(StringTool.tocapitalizeFirstLetter(iteratestack.peek().toString()),
        node.getType());
